...
//Création du HTML après appel de l'API
	...
	async function allProductsList(){
        ...
        // Ajout des attributs
        produitBlock.setAttribute("class", "productList_block");
        produitLeft.setAttribute("class", "productList_block--left");
        produitRight.setAttribute("class", "productList_block--right");
        produitImage.setAttribute("src", produit.imageUrl);
        produitImage.setAttribute("alt", "image de l'appareil photo");
        ...
        //Block conteneur en flex
         ...
         produitLink.textContent = "Voir la fiche produit"
        });
    }    
...
async function detailProduit(){
    ...
    //Selon le type de produit (ligne 4) création des options
    switch(produitSell){
    	case "cameras":
    ...
    let formVille = document.getElementById("formVille").value;
      ...
      //tests des différents input du formulaire
        ...
        if(checkNumber.test(formNom) == true || checkSpecialCharacter.test(formNom) == true || formNom == ""){
        	checkMessage = "Veuillez renseigner votre nom";
        }else{
...
//Envoi du formulaire
  ...
  envoiDonnees = (objetRequest) => {
  	return new Promise((resolve)=>{
          ...
          document.forms["form-panier"].action = './validation.html';
